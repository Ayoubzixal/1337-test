    ...
    <div className="min-h-screen bg-black flex items-center justify-center font-mono overflow-auto p-2 sm:p-4">
      <div className="w-full max-w-[1024px] min-h-[calc(100vh-16px)] sm:min-h-0 sm:h-[768px] bg-[#050505] p-4 sm:p-8 flex flex-col relative overflow-hidden border border-[#00FF41]/10">
        ...
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 border-b border-[#00FF41]/30 pb-4 z-10 shrink-0">
          ...
          <div className="text-left sm:text-right w-full sm:w-auto">
            <div className="flex items-center gap-2 mb-2 sm:mb-1 justify-between sm:justify-end border-b border-[#00FF41]/20 sm:border-none pb-2 sm:pb-0">
              ...
              <span className="text-xs sm:text-sm font-bold text-[#00FF41]">AYOUB & RACHID</span>
            </div>
            <div className="flex items-center gap-4 justify-between sm:justify-end">
              <Link to="/admin" className="text-[10px] text-[#00FF41]/60 hover:text-[#00FF41] hover:underline underline-offset-4">
                ADMIN_ACCESS
              </Link>
              <div className="admin-badge inline-block text-[10px] sm:text-xs px-2 py-0.5">SYSTEM ACTIVE</div>
            </div>
          </div>
        </header>
        ...
        <footer className="mt-6 flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-end border-t border-[#00FF41]/30 pt-4 text-[10px] uppercase z-10 text-[#00FF41] shrink-0">
          ...
          <div className="text-left sm:text-right w-full sm:w-auto">
            <span>PROPERTY OF ELITE_WHITE_HAT_CORP</span><br className="hidden sm:block"/>
            <span className="text-[#00FF41]/40 block sm:inline mt-1 sm:mt-0">DEVELOPED BY AYOUB & RACHID SECURITY LABS</span>
          </div>
        </footer>
      </div>
    </div>
        ...
        <div className="flex-1 p-2 sm:p-6 overflow-hidden flex flex-col">
          {!state ? (
            <div className="flex justify-center items-center h-full flex-1">
              <span className="text-[#00FF41] animate-pulse text-xs sm:text-sm">CONNECTING TO SECURE MAINFRAME...</span>
            </div>
          ) : !state.isActive ? (
            <div className="flex flex-col justify-center items-center h-full flex-1 text-center p-4">
              ...
              <p className="text-[#00FF41] font-bold uppercase tracking-widest text-base sm:text-lg">Waiting for Admin Signal</p>
              <p className="text-[#00FF41]/60 text-xs sm:text-sm mt-2">SYSTEM STANDBY <span className="animate-blink">_</span></p>
            </div>
          ) : (
             <motion.div 
                  ...
                  <table className="w-full text-left border-collapse block overflow-y-auto h-full pr-0 sm:pr-2">
                    <thead className="text-[10px] sm:text-[11px] uppercase text-[#00FF41]/50 border-b border-[#00FF41]/30 table w-full table-fixed sticky top-0 bg-[#050505]">
                      <tr>
                        ...
                        <th className="pb-3 font-medium w-32 sm:w-64">LOAD_CAPACITY</th>
                        <th className="pb-3 font-medium w-24 sm:w-32">THREAT_LEVEL</th>
                        <th className="pb-3 font-medium text-right w-32 sm:w-48">ACTION</th>
                      </tr>
                    </thead>
                      ...
                      {state.rooms.map((room, index) => {
                        ...
                        return (
                          <tr key={room.id} className={cn("room-row table w-full table-fixed", isLast && "border-none")}>
                            <td className="py-3 sm:py-4 font-bold w-12 sm:w-16">0{room.id}</td>
                            <td className="py-3 sm:py-4 truncate pr-2">{room.name}</td>
                            <td className="py-3 sm:py-4 w-32 sm:w-64 pr-2">
                              <div className={cn("flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2", isFull ? "text-[#00FF41]/60" : "text-[#00FF41]")}>
                                <span className="text-[10px] sm:text-sm whitespace-nowrap">{room.guests} / {room.max}</span> 
                            ...
                            <td className="py-3 sm:py-4 text-[10px] sm:text-xs w-24 sm:w-32">{threatLevels[(room.id - 1) % 4]}</td>
    ...
    <MainLayout>
      <div className="w-full max-w-[860px] z-10 relative">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-6 gap-2 border-b border-[#00FF41]/20 sm:border-none pb-4 sm:pb-0">
          <h2 className="text-xl sm:text-2xl font-bold uppercase text-[#00FF41]">Control Panel</h2>
          <button 
            onClick={logout}
        ...
        <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
          <div className="glow-border bg-black/40 backdrop-blur-sm p-4 sm:p-6 flex flex-col items-center justify-center text-center h-48 sm:h-64">
            <h3 className="text-sm sm:text-lg text-[#00FF41]/60 mb-2 sm:mb-4 uppercase">Main Window Status</h3>
            <div className={cn(
          ...
          <div className="glow-border bg-black/40 backdrop-blur-sm p-4 sm:p-6 h-48 sm:h-64 flex flex-col items-stretch overflow-hidden text-[#00FF41]">
            <h3 className="text-xs sm:text-sm text-[#00FF41]/60 mb-2 sm:mb-4 uppercase inline-block border-b border-[#00FF41]/40 pb-2 self-start shrink-0">Live Room Telemetry</h3>
            <div className="space-y-2 sm:space-y-3 overflow-y-auto pr-2 flex-1">
              {!state ? (
                <div className="text-xs sm:text-sm text-[#003B00]">No telemetry...</div>
              ) : state.rooms.map(room => (
                <div key={room.id} className="flex justify-between items-center text-xs sm:text-sm border-b border-[#00FF41]/20 pb-2">
                  <span className="text-[#00FF41] truncate pr-2 max-w-[100px] sm:max-w-[150px]">{room.name}</span>
                  <div className="flex items-center gap-2 sm:gap-4 shrink-0">
                    ...
                    <span className="text-[#00FF41] w-10 sm:w-12 text-right font-bold text-[10px] sm:text-sm">{room.guests}/{room.max}</span>
                  </div>
                </div>
              ))}
            </div>
